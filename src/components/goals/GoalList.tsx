        ...
        <TableBody>
          {goals.map((g) => (
            <TableRow key={g.id}>
              ...
              <TableCell className="font-mono tabular-nums">{g.weightage}%</TableCell>
              <TableCell className="text-right">
                <div className="flex justify-end gap-1">
                  {g.is_locked && <Lock className="h-3.5 w-3.5 text-muted-foreground self-center" />}
                  {editable && !g.is_locked && onUpdate && (
                    <Button
                      variant="ghost"
                      size="sm"
                  ...
                  <Input
                    ...
                    onBlur={(e) => {
                      ...
                      if (v >= 10 && v <= 100 && v !== link.weightage) {
                        void onUpdateSharedWeightage(link.id, v);
                      } else {
